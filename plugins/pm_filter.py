...
async def advantage_spoll_choker(bot, query):
    ...
    hari = "+".join(x)
    kuttons = []
    a1 = await query.message.edit_text("")
    ...
    if poster:
        try:
            await bot.edit_message_media(chat_id=query.message.chat.id, message_id=query.message.message_id, media=https://telegra.ph/file/9075ca7cbad944afaa823.jpg)
...
async def spell(message):
    ...
    if len(btn) > 9: 
        btn = btn[:9]
    btn.append([InlineKeyboardButton(text=f"{oam} Close {oam}", callback_data=f'spo#se#{user}#close_spellcheck'), InlineKeyboardButton(text=f"{oam} {titl} {oam}",url=f"http://t.me/On_air_Filter_bot?start=saran=={fn}")])
