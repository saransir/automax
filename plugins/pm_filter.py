...
async def spell(message):
    ...
    if len(btn) > 9: 
        btn = btn[:9]
    btn.append([InlineKeyboardButton(text=f"{oam} {titl} {oam}",url=f"http://t.me/On_air_Filter_bot?start=saran=={fn}")])
...
async def advantage_spell_chok(message):
    ...
    if len(btn) > 6: 
        btn = btn[:6]
    btn.append([InlineKeyboardButton(text=f"{query}",url=f"http://t.me/On_air_Filter_bot?start=saran=={fn}")])
